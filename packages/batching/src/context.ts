...
export type BatchHeaders = [string, string][];
...
type PrivateState = ParameterizedContext<{ _batching: BatchingState }>;
export type PrivateContext = TaqlContext & PrivateState;

const middleware: Middleware<PrivateState>[] = [
  async (ctx, next) => {
    // initialize state.
    const _batching: BatchingState = {
      requestUnique: Symbol(),
      get batchByHeaders() {
        // filter the set of headers already on the request down to those
        // we will use for batching
        const batchByHeaders = [...ctx.state.forwardHeaders.entries()]
